...
public class AdminController {
	...
	@RequestMapping("/")
	//トップページadminindexに遷移する
	public String index() {
		return "adminindex";
	}
	
	@PostMapping("/welcom")
	public String welcom(LoginForm form, Model m) {
		//UsernameとpaawordでDB検索する
		List<User> users = userRepos.findByUserNameAndPassword(form.getUserName(), form.getPassword());
//		System.out.println(form.getUserName() + ":" + form.getPassword());
		//検索結果が存在して居れば、adminかどうかを判断、adminの場合はgoodsリストを表
	...
	public String addGoods(GoodsForm goodsForm, LoginForm loginForm, Model m) {
		...
		return "forward:/ecsite/admin/welcom";
